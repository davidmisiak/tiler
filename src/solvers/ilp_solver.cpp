...
IlpSolver::IlpSolver(Problem problem, std::unique_ptr<CoinCbcWrapper> coin_cbc_wrapper)
        : problem_(problem), coin_cbc_wrapper_(std::move(coin_cbc_wrapper)) {
...
Solution IlpSolver::solve(bool print_stats) {
    ...
    for (const Tile& tile : problem_.tiles_) {
        ...
        for (auto [bx, by] : problem_.board_.get_cells()) {
            for (const Region& region : tile) {
                ...
                Var var = coin_cbc_wrapper_->new_var(true, 0.0, 1.0, 0.0);
                tile_vars.push_back(var.with_coeff(1.0));
                for (auto [rx, ry] : region.get_cells()) {
                    cell_vars[sy + ry][sx + rx].push_back(var);
                }
            }
        }
    ...
    for (auto [x, y] : problem_.board_.get_cells()) {
        ...
        coin_cbc_wrapper_->add_constraint(Constraint(cell_vars[y][x], ConstraintSense::kEq, 1.0));
    }
    ...
    bool result = coin_cbc_wrapper_->solve(ObjectiveSense::kIgnore);
    if (!result) return {};

    Solution solution;
    auto [_, var_values] = coin_cbc_wrapper_->get_solution();
    for (int i = 0; i < static_cast<int>(placed_regions.size()); i++) {
        if (var_values[i] < 0.5) continue;
